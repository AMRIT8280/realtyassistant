...
$sql_list_about = "SELECT * FROM ".DB_DIRECTORIES_CATEGORY."  WHERE 1 ".$con." AND status!='D' ORDER BY id DESC";	
$qry_list_about = mysqli_query($link,$sql_list_about);
$num_list_about = mysqli_num_rows($qry_list_about);


$countShow="SELECT count(*) FROM ".DB_DIRECTORIES_CATEGORY."  WHERE 1 ".$con." AND status!='D' ORDER BY id DESC";
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-lg-12 breadcrumb-header">
        <div id="breadcrumb" class="tooltip-demo"> <a href="<?=URL?>" title="Go to Home" data-toggle="tooltip" data-placement="bottom"><i class="fa fa-home fa-fw"></i> Home</a><a href="#" class="current">Directories Category</a></div>
      </div>
    </div>
    <!-- /.row -->
    <div class="row">
      <div class="col-lg-12">
        <div class="panel tabbed-panel panel-success">
          <div class="panel-heading clearfix">
            <div class="panel-title pull-left"><a data-toggle="collapse" href="#collapseOne" aria-expanded="false"><strong>Directories Category Details</strong></a></div>
                      ...
                      <table id="dataTables-example" class="table table-striped table-bordered">
						...
						?>
                          <tr>
                            ...
                            <td>
								<img src="../../uploads/directories/<?php echo $row_list_about['image_name']?>" width="70px" />
                            </td>
...
{
	var UserResp = window.confirm("Are you sure to delete this Directories Category?");
	if( UserResp == true )
	{
                    ...
                    <div class="table-responsive">
                      <table width="100%">
                        <tbody>
                          ...
                          <tr>
                            <td width="15%">Category Name <font color="#ff0000">*</font></td>
                            <td width="3%">&nbsp;</td>
                            <td width="82%"><div class="col-lg-6">
                                <div class="form-group">
                                  <input type="text" class="form-control" id="main_title" name="main_title" placeholder="Enter board basis name" autocomplete="off">
                                  </div>
                                <!-- /input-group --> 
                              </div></td>
                          </tr>
                          <tr>
                            ...
                            <td width="82%"><div class="col-lg-6">
                                <div class="form-group">
                                  <input type="text" class="form-control" id="sub_title" name="sub_title" placeholder="Enter board basis title" autocomplete="off">
                                  </div>
                                <!-- /input-group --> 
                              </div></td>
                          </tr>
                          ...
                          <tr>
                            <td colspan="3"><input type="hidden" name="id" value="">
                              ...
                              <a href="manage_directories_category.php" class="btn btn-default">Cancel</a></td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
...
function insert_about(){
	global $link;	
		...
		$upload_dir='../../uploads/directories/';
	 ...
	 $sql_inset_post="INSERT INTO ".DB_DIRECTORIES_CATEGORY." 

		SET 
		image_name='".$file_name_image."',	
		main_title = '".ConvertRealString($_REQUEST['main_title'])."',
		sub_title = '".ConvertRealString($_REQUEST['sub_title'])."'";
	...
	$qry_inset_post	=mysqli_query($link,$sql_inset_post);	
		...
		$_SESSION['DIV_CLASS'] ="alert-success"; 
		 ...
		 redirect_mp('manage_directories_category');
	exit();
...
global $link;
		$sql_delete_about= "UPDATE ".DB_DIRECTORIES_CATEGORY." 
                    SET 
					status= 'D'
                    where id='".$_REQUEST['id']."'";   
		$qry_delete_about=mysqli_query($link,$sql_delete_about) or die(mysqli_error());
		
		$_SESSION['MSG_ALERT'] = "Latest sub stream has been successfully deleted.";
				...
				if($val!='on')
					{
							...
							$sql_deletepublication = "UPDATE ".DB_DIRECTORIES_CATEGORY." 
											SET 
											status= 'D'
											where id='".$val."'";
...
function edit(){
	...
	global $link;
$sql_edit_about = "SELECT * FROM ".DB_DIRECTORIES_CATEGORY."  where id ='".$_REQUEST['id']."'";
...
<div id="page-wrapper">
  <div class="container-fluid">
    <div class="row">
      <div class="col-lg-12 breadcrumb-header">
        <div id="breadcrumb" class="tooltip-demo"> <a href="<?=URL?>" title="Go to Home" data-toggle="tooltip" data-placement="bottom"><i class="fa fa-home fa-fw"></i> Home</a><a href="<?=$_SERVER['PHP_SELF']?>">Directories Category</a><a href="#" class="current">Edit</a></div>
      </div>
    </div>
    <!-- /.row -->
    <div class="row">
      <div class="col-lg-12">
        <div class="panel tabbed-panel panel-success">
          <div class="panel-heading clearfix">
            <div class="panel-title pull-left"><a data-toggle="collapse" href="#collapseOne" aria-expanded="false"><strong><i class="fa fa-edit fa-fw"></i> Edit sub stream</strong></a></div>
          </div>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td width="87%"><div class="col-lg-6">
                            ...
                            <p id="image_name_alert" style="color:#F00;"></p>
                            <?php if(!empty($row_edit_about['image_name'])){?>
                            <div class="form-group"><img src="../../uploads/directories/<?php echo $row_edit_about['image_name']?>" width="100px" /></div>
                            <?php }?>
                          </div></td>
                      </tr>
                      <tr>
		...
		$upload_dir='../../uploads/directories/';
			...
			if($_FILES['image_name']['name']!="")
			{
				$file_explode = explode(".",$_FILES['image_name']['name']);
					...
					$tmp_name=$_FILES['image_name']['tmp_name'];
					move_uploaded_file($tmp_name,$upload_dir.$file_name_image);
					$sql_inset_post="UPDATE ".DB_DIRECTORIES_CATEGORY." 
                    SET 
					image_name='".$file_name_image."'
                    where id='".$_REQUEST['id']."'";
					$qry_inset_post	=mysqli_query($link,$sql_inset_post);
...
//--------------------------------- Insert validation   ---------------------------------
		...
		messages: {
			main_title: "Please enter board basis name...!",
			sub_title: "Please enter board title...!"
		},
		errorElement: 'label',
